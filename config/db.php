<?php

return [
  "restaurants" => [
    [
      "name" => "Mc Donald's",
      "image" => "https://www.coolinmilan.it/wp-content/uploads/2022/03/mcdonalds-milano.jpg",
    ],
    [
      "name" => "Burger King",
      "image" =>
      "https://i0.wp.com/www.fruitbookmagazine.it/wp-content/uploads/2021/01/burger-king-rebranding.jpg?fit=1200%2C630&ssl=1",
    ],
    [
      "name" => "KFC",
      "image" =>
      "https://media-cdn.tripadvisor.com/media/photo-s/1b/99/44/8e/kfc-faxafeni.jpg",
    ],
    [
      "name" => "KebHouze",
      "image" =>
      "https://kebhouze.com/wp-content/uploads/2023/05/kiosk1-1-2048x1365.jpg",
    ],
    [
      "name" => "Poke House",
      "image" =>
      "https://just-eat-prod-eu-res.cloudinary.com/image/upload/c_fill,f_auto,q_auto,w_1200,h_630,d_it:cuisines:poke-2.jpg/v1/it/restaurants/229462.jpg",
    ],
    [
      "name" => "Billy Tacos",
      "image" =>
      "https://www.confimprese.it/wp-content/uploads/2022/10/Billy-Tacos-Maximo.jpg",
    ],
    [
      "name" => "Gino Sorbillo",
      "image" =>
      "https://thumbs.dreamstime.com/z/logo-sorbillo-della-pizzeria-pi%C3%B9-importante-del-mondo-169971641.jpg",
    ],
    [
      "name" => "Rossopomodoro",
      "image" =>
      "https://www.romaest.cc/fileadmin/user_upload/GLOBAL/brand_stores/logos/rossopomodoro.jpg",
    ],
    [
      "name" => "Sushiko",
      "image" =>
      "https://sushiko.it/wp-content/uploads/2022/01/Logo-Sushiko.png",
    ],
    [
      "name" => "Wakame",
      "image" =>
      "https://wakameitalia.it/wp-content/uploads/2020/04/riduzione-logo-wakame-selection.png",
    ]
  ],
  "dishes" => [
    [
      "restaurant_id" => 1,
      "name" => "Big Mac",
      "description" => "Il Grande Classico di McDonald’s è pronto a stupirti con il suo gusto ancora più irresistibile. Lasciati avvolgere dal pane più caldo, trasportare dal sapore della sua carne più succosa e goditi un’ulteriore aggiunta della sua inconfondibile salsa: lo scoprirai ancora più buono.",
      "image" => "https://www.mcdonalds.it/sites/default/files/styles/product_isolated_preview/public/products/big-mac-isolated.png?itok=jQEbyoQJ",
      "visibility" => true
    ],
    [
      "restaurant_id" => 1,
      "name" => "McChicken",
      "description" => "Tutta la semplicità del petto di pollo avvolto in una panatura croccante, insieme all’insalata iceberg e all’inconfondibile salsa McChicken.",
      "image" => "https://mcdonalds.it/sites/default/files/styles/product_isolated_preview/public/products/mcchicken--hero-isolated.png?itok=_8uEiFFU",
      "visibility" => true
    ],
    [
      "restaurant_id" => 2,
      "name" => "Bacon King 3.0",
      "description" => "L'evoluzione del Bacon King: 3 strati di carne alla griglia, 3 strati di formaggio e 8 fette di delizioso bacon.",
      "image" => "https://www.burgerking.it/site/assets/files/8495654/bacon_king_3_0-1.png",
      "visibility" => true
    ],
    [
      "restaurant_id" => 2,
      "name" => "Grilled Chicken Royale",
      "description" => "Panino con petto di pollo grigliato Fileni 100% Italiano, il mix perfetto per chi vuole consumare cibo genuino senza rinunciare al gusto! I polli sono allevati a terra e vengono nutriti con alimenti 100% italiani.",
      "image" => "https://www.burgerking.it/site/assets/files/9008273/app_kiosk_burger_transp.png",
      "visibility" => true
    ],
    [
      "restaurant_id" => 3,
      "name" => "Double Colonel's Burger",
      "description" => "Il Double Colonel’s Burger ha due filetti di pollo preparati con la ricetta segreta Original: così buono buono il doppio doppio.",
      // "image" => "https://media.engage.it/2023/4/kfc-italia-doublecolonel_734519.jpg",
      "image" => "https://media.imagonist.com/products/products/312_view_it.png?v=1688399201",
      "visibility" => true
    ],
    [
      "restaurant_id" => 3,
      "name" => "Double Krunch",
      "description" => "La morbida croccantezza raddoppia con 2 Tender Crispy! Pane con semi di sesamo, due Tender Crispy, insalata, cheddar e salsa al pepe nero.",
      // "image" => "https://www.laurabustarviejo.com/wp-content/uploads/2017/05/Nuevo-Double-Krunch-KFC.jpg",
      "image" => "https://media.imagonist.com/products/products/282_view_it.png?v=1688397846",
      "visibility" => true
    ],
    [
      "restaurant_id" => 4,
      "name" => "Burrito",
      "description" => "Il burrito è una pietanza che appartiene alla cucina tex-mex e consiste in una tortilla di farina di grano riempita con carne di bovino, pollo o maiale, che è poi chiusa ottenendo una forma sottile.",
      "image" => "https://www.fattoincasadabenedetta.it/wp-content/uploads/2023/02/Burrito0105.jpg",
      "visibility" => true
    ],
    [
      "restaurant_id" => 4,
      "name" => "Mex Tacos",
      "description" => "I taco tex-mex hanno un ripieno più standardizzato che consiste di carne macinata cotta in spezie considerate messicane, formaggio grattugiato, pezzetti di pomodoro crudo, lattuga e salsa di pomodoro più o meno piccante.",
      "image" => "https://www.melarossa.it/wp-content/uploads/2020/12/tacos.jpg",
      "visibility" => true
    ],
    [
      "restaurant_id" => 5,
      "name" => "Sunny Salmon",
      "description" => "Riso Bianco, Salmone Condito, Avocado, Edamame, Cavolo Rosso, Salsa Speciale, Crema di Avocado, Sesamo",
      "image" => "https://media.poke-house.com/wp-content/uploads/2022/05/POKE_HOUSE_HOUSE_BOWLS_ita_sunny_salmon-150x150.jpg",
      "visibility" => true
    ],
    [
      "restaurant_id" => 5,
      "name" => "Super Protein",
      "description" => "Quinoa e Fresh Baby Spinach, Pollo Teriyaki e Zenzero, Edamame, Avocado Hass, Pomodorini Datterino, Olio al Limone, Grana Padano a Scaglie, Mandorle, Sesamo",
      "image" => "https://media.poke-house.com/wp-content/uploads/2023/05/POKE_HOUSE_HOUSE_BOWL_super_protein_IT-150x150.jpg",
      "visibility" => true
    ],
    [
      "restaurant_id" => 6,
      "name" => "Burrito Mexican Sausage",
      "description" => "Tortilla classica, riso, fagioli, mexican sausage, insalata Billy e pico de gallo.",
      "image" => "https://res.cloudinary.com/glovoapp/w_340,h_170,c_fit,f_auto,q_auto/Products/jqqcs14eaider4esx5af",
      "visibility" => true
    ],
    [
      "restaurant_id" => 6,
      "name" => "French Tacos con hamburger",
      "description" => "Tortilla classica, patate fritte, hamburger, cheddar e maionese.",
      "image" => "https://res.cloudinary.com/glovoapp/w_340,h_170,c_fit,f_auto,q_auto/Products/zkzpwmxocejhvthy2hnm",
      "visibility" => true
    ],
    [
      "restaurant_id" => 7,
      "name" => "Antica Margherita",
      "description" => "Pomodoro Italiano di Agricoltura Biologica, Fiordilattw, Olio Evo Nobilis Terre Francescane Estratto a Freddo, Basilico Fresco, doppia mozzarella",
      "image" => "https://images.ctfassets.net/nw5k25xfqsik/64VwvKFqxMWQORE10Tn8pY/200c0538099dc4d1cf62fd07ce59c2af/20220211142754-margherita-9920.jpg",
      "visibility" => true
    ],
    [
      "restaurant_id" => 7,
      "name" => "Marinara",
      "description" => "Pomodoro di Agricoltura Biologica, Origano, Aglio e Olio EVO Italiano",
      "image" => "https://cdn.tasteatlas.com/images/recipes/283708d6bafd4773abde118b4ba69fa9.jpg",
      "visibility" => true
    ],
    [
      "restaurant_id" => 8,
      "name" => "Verace",
      "description" => "pomodoro Roma, mozzarella bufala DOP, basilico, in uscita dal forno: olio evo della Penisola Sorrentina DOP",
      "image" => "https://www.sebeto.com/intranet/ftpscambio/fotografier/3.jpg",
      "visibility" => true
    ],
    [
      "restaurant_id" => 8,
      "name" => "Rossopomodoro",
      "description" => "pomodoro San Marzano DOP, provola affumicata, pomodoro Piennolo del Vesuvio DOP, datterino giallo di Battipaglia e, in uscita dal forno, scaglie di cacioricotta stagionato",
      "image" => "https://www.sebeto.com/intranet/ftpscambio/fotografier/PI345.jpg",
      "visibility" => true
    ],
    [
      "restaurant_id" => 9,
      "name" => "Uramaki Ghibli",
      "description" => "Gamberone in tempura, riso soffiato, tartare di salmone*, salsa teriyaki, avocado",
      "image" => "https://www.marshalsushi.it/wp-content/uploads/2017/02/72-1200x1200.jpg",
      "visibility" => true
    ],
    [
      "restaurant_id" => 9,
      "name" => "Uramaki Drago",
      "description" => "Gamberone* in tempura, avocado, maionese, tobiko*, salsa teriyaki",
      "image" => "https://www.samurai-sushi.it/wp-content/uploads/2016/10/084-dragon-roll_DSC_5952-scaled.jpg",
      "visibility" => true
    ],
    [
      "restaurant_id" => 10,
      "name" => "Piri Piri Crunch",
      "description" => "8x California al salmone cotto con Crunch-Topping.",
      "image" => "https://wakameitalia.it/wp-content/uploads/2020/11/piri-piri-1.png",
      "visibility" => true
    ],
    [
      "restaurant_id" =>  10,
      "name" => "Gyoza Pollo",
      "description" => "5x Ravioli di pollo e crema di sesamo.",
      "image" => "https://wakameitalia.it/wp-content/uploads/2020/04/9-gyoza-pollo.png",
      "visibility" => true
    ],
  ],
  "types" => [
    [
      "name" => "Italiana",
      "icon" => "https://images.pexels.com/photos/708587/pexels-photo-708587.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=2"
    ],
    [
      "name" => "Americana",
      "icon" => "https://images.pexels.com/photos/1251198/pexels-photo-1251198.jpeg?auto=compress&cs=tinysrgb&w=800"
    ],
    [
      "name" => "Turca",
      "icon" => "https://images.pexels.com/photos/7866189/pexels-photo-7866189.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=2"
    ],
    [
      "name" => "Cinese",
      "icon" => "https://images.pexels.com/photos/12955612/pexels-photo-12955612.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=2"
    ],
    [
      "name" => "Messicana",
      "icon" => "https://images.pexels.com/photos/4968309/pexels-photo-4968309.jpeg"
    ],
  ]
];
